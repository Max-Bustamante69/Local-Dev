...
export default class OrthofixOrderForm extends NavigationMixin(
  ...
  @track doesFileExist = false;

  @wire(getCurrentUser)
  wiredUser({ error, data }) {
    if (data) {
      this.currentUser = data;
      this.checkUserRoleFitter =
        this.currentUser.UserRole.Name === "BGT Fitter";
  ...
  connectedCallback() {
    ...
    this.setObserver(orderFormTitle);
  }
  ...
  getOrder() {
    ...
    getOrder({ id: this.recordId })
      .then((result) => {
        ...
        this.headerText =
          this.headerText +
          orderData.patientInformation.productType +
          "™" +
          " " +
          " - " +
          " " +
          orderData.patientInformation.firstName +
          "  " +
          orderData.patientInformation.lastName;
        ...
        if (orderData.Status === "Submitted To IA") {
          ...
          if (
            ...
            orderData.isChannelScriptOrder === true
          ) {
        ...
        [
          ...
          "plannedTreatmentGraftSource",
          "previousTreatment",
        ].forEach((picklistName) => {
          this.formData[picklistName] = !!this.formData[picklistName]
            ? this.formData[picklistName].split(";")
            : [];
        });
        this.editedData = orderData;

        this.handleAddFavoritesButton({
          fieldName: "patientInformation.prescriber.firstName",
        });
  ...
  handleTabsValidation() {
    ...
    if (
      ...
      (unfulfilledTab === "" || this.isChannelScriptOrder === true)
    ) {
  ...
  handleSubmitToIA() {
    ...
    if (
      ...
      showError(
        this,
        "Validation Error",
        "Please upload a Prescription before submitting the Order."
      );
      return;
    }
    this.handleTabsValidation();
    ...
    if (
      ...
      showError(
        this,
        "Validation Error",
        "Please fill in all required fields before submitting the order"
      );
      return;
    }
  }
  ...
  handleFilesCheck(event) {
    ...
    console.log("inside preciption checked or no", this.doesFileExist);
  }
  ...
  handleMandatoryFieldsValidate() {
    ...
    if (this.activeTab === "patientTab") {
      ...
      let patientInformation = this.validateItem(
        this.requiredFields.patientInformation,
        this.editedData,
        "patientInformation"
      );
      missedFields = [...missedFields, ...patientInformation];

      let prescriber = this.validateItem(
        this.requiredFields.patientInformation.prescriber,
        this.editedData.patientInformation,
        "prescriber"
      );
    ...
    if (this.activeTab === "clinicalTab") {
      if (!this.editedData.patientInformation) {
        ...
        if (
          this.editedData.patientInformation.productType == "AccelStim" ||
          this.editedData.patientInformation.productType == "PhysioStim"
        ) {
        ...
        if (
          this.editedData.patientInformation.productType == "CervicalStim" ||
          this.editedData.patientInformation.productType == "SpinalStim"
        ) {
  ...
  save(showSuccessMsg) {
    ...
    if (
      ...
      if (
        !orderData.patientInformation.prescriber ||
        this.isObjectEmpty(orderData.patientInformation.prescriber)
      ) {
      ...
      if (
        !orderData.patientInformation.emergencyContact ||
        this.isObjectEmpty(orderData.patientInformation.emergencyContact)
      ) {
        orderData.patientInformation.emergencyContact = null;
      }
      if (
        !orderData.patientInformation.primaryCarePhysician ||
        this.isObjectEmpty(orderData.patientInformation.primaryCarePhysician)
      ) {
      ...
      console.log(
        "Saved patientInformation",
        JSON.stringify(patientInformation)
      );
      console.log(
        "save insurance info:" + JSON.stringify(this.getInsuranceSaveData())
      );
      ...
      saveOrder({ order: patientInformation })
        .then((result) => {
          ...
          if (
            ...
            this.editedData.riskFactors.slice(-1) == ";"
          ) {
            this.editedData.riskFactors = this.editedData.riskFactors.slice(
              0,
              -1
            );
          }
          let clinicalInformationData = {
            ...
            revisionSurgeryGraftSource: (
              this.editedData.revisionSurgeryGraftSource || []
            ).join(";"),
            revisionSurgery: (this.editedData.revisionSurgery || []).join(";"),
            initialSurgeryGraftSource: (
              this.editedData.initialSurgeryGraftSource || []
            ).join(";"),
            ...
            plannedTreatmentGraftSource: (
              this.editedData.plannedTreatmentGraftSource || []
            ).join(";"),
          ...
          console.log(
            ...
            JSON.stringify(clinicalInformationData)
          );
  ...
  prepareEventData(editedData) {
    console.log(
      "editedData.eventInformation.appointmentdate",
      editedData.eventInformation
    );
    if (
      editedData.eventInformation &&
      editedData.eventInformation.appointmentdate &&
      editedData.eventInformation !== undefined
    ) {
      ...
      showError(
        this,
        "Validation Error",
        "Please enter appointment date and time to submit"
      );
    }
  }
    ...
    if (this.editedData && this.editedData.eventInformation) {
      ...
      createEvent({ eventData })
        .then((result) => {
          ...
          this.makeEventReadOnly = true;
          this.showSubmit = false;
          const scheduleAppointmentTabComp = this.template.querySelector(
            "c-orthofix-order-form-schedule-appointment"
          );
  ...
  handleListViewNavigation() {
    window.location.href =
      "https://demoargano--orthodev.sandbox.lightning.force.com/lightning/page/home";
  }
  ...
  saveAndSubmit() {
    ...
    if (
      !orderData.patientInformation.altContact ||
      this.isObjectEmpty(orderData.patientInformation.altContact)
    ) {
    ...
    if (!insuranceData) {
      ...
      orderData.secondary = insuranceData.secondary
        ? insuranceData.secondary
        : null;
      orderData.tertiary = insuranceData.tertiary
        ? insuranceData.tertiary
        : null;
      orderData.quaternary = insuranceData.quaternary
        ? insuranceData.quaternary
        : null;
    }
  ...
  checkInsuranceItem(insuranceData, objectName, innerObjectName) {
    if (
      this.editedData[objectName] &&
      !this.isObjectEmpty(this.editedData[objectName][innerObjectName])
    ) {
      if (!insuranceData[objectName]) {
        //insuranceData[objectName] = {};
        insuranceData[objectName] = {
          id: this.editedData[objectName].id
            ? this.editedData[objectName].id
            : null,
          insuranceType: this.editedData[objectName].insuranceType,
        };
      }
      insuranceData[objectName][innerObjectName] =
        this.editedData[objectName][innerObjectName];
    }
  }
  ...
  async checkIfPrescriberIsFavorite() {
    console.log(
      new Date().getTime() - this.lastTimeCheckFavoriteRequested >= 500
    );
    if (new Date().getTime() - this.lastTimeCheckFavoriteRequested >= 500) {
      if (!this.editedData.patientInformation.prescriber.isFavorite) {
        this.editedData.patientInformation.prescriber.isFavorite =
          await isFavorite({
            physitian: this.editedData.patientInformation.prescriber,
          });
        this.showAddFavoriteButton =
          !this.editedData.patientInformation.prescriber.isFavorite;
  ...
  async checkIfInsuranceIsFavorite(detail) {
    if (new Date().getTime() - this.lastTimeCheckFavoriteRequested >= 500) {
      if (
        !this.editedData.primary.isFavorite ||
        !this.editedData.secondary.isFavorite ||
        !this.editedData.tertiary.isFavorite ||
        !this.editedData.quaternary.isFavorite
      ) {
        if (
          detail.fieldName === "primary.carrier.name" ||
          detail.fieldName === "primary.insuranceType"
        ) {
          this.editedData.primary.isFavorite = await isFavoriteInsurance({
            insurance: this.editedData.primary,
          });
          this.showAddFavoriteInsuranceButton =
            !this.editedData.primary.isFavorite;
        } else if (
          detail.fieldName === "secondary.carrier.name" ||
          detail.fieldName === "secondary.insuranceType"
        ) {
          this.editedData.secondary.isFavorite = await isFavoriteInsurance({
            insurance: this.editedData.secondary,
          });
          this.showAddFavoriteInsuranceButton =
            !this.editedData.secondary.isFavorite;
        } else if (
          detail.fieldName === "tertiary.carrier.name" ||
          detail.fieldName === "tertiary.insuranceType"
        ) {
          this.editedData.tertiary.isFavorite = await isFavoriteInsurance({
            insurance: this.editedData.tertiary,
          });
          this.showAddFavoriteInsuranceButton =
            !this.editedData.tertiary.isFavorite;
        } else if (
          detail.fieldName === "quaternary.carrier.name" ||
          detail.fieldName === "quaternary.insuranceType"
        ) {
          this.editedData.quaternary.isFavorite = await isFavoriteInsurance({
            insurance: this.editedData.quaternary,
          });
          this.showAddFavoriteInsuranceButton =
            !this.editedData.quaternary.isFavorite;
        }
  ...
  handleAddFavoritesButton(detail) {
    const favFields = [
      "patientInformation.prescriber.firstName",
      "patientInformation.prescriber.lastName",
      "patientInformation.prescriber.npi",
    ];
    if (favFields.includes(detail.fieldName)) {
      console.log("Fav tracking field changed", detail.fieldName);
      if (
        !!this.editedData.patientInformation.prescriber.firstName &&
        !!this.editedData.patientInformation.prescriber.lastName &&
        !!this.editedData.patientInformation.prescriber.npi
      ) {
  ...
  handleAddFavoritesInsuranceButton(detail) {
    const favInsurance = [
      ...
      "secondary.insuranceType",
      "tertiary.carrier.name",
      "tertiary.insuranceType",
      "quaternary.carrier.name",
      "quaternary.insuranceType",
    ];
    if (favInsurance.includes(detail.fieldName)) {
      console.log("check add fav insurance");
      if (
        (!!this.editedData.primary?.insuranceType &&
          !!this.editedData.primary?.carrier?.name) ||
        (!!this.editedData.secondary?.insuranceType &&
          !!this.editedData.secondary?.carrier?.name) ||
        (!!this.editedData.tertiary?.insuranceType &&
          !!this.editedData.tertiary?.carrier?.name) ||
        (!!this.editedData.quaternary?.insuranceType &&
          !!this.editedData.quaternary?.carrier?.name)
      ) {
  ...
  handleGenericInputChange(event) {
    ...
    if (arr.length == 1) {
      ...
      if (fieldName === "productType") {
        if (
          typeof this.formData.patientInformation.firstName === "undefined" &&
          typeof this.formData.patientInformation.lastName === "undefined"
        ) {
          ...
          console.log(
            "this.formData.patientInformation.firstName   " +
              JSON.stringify(this.formData.patientInformation.firstName)
          );
          console.log(
            "this.formData.patientInformation.lastnae   " +
              JSON.stringify(this.formData.patientInformation.lastName)
          );
      ...
      if (fieldName === "firstName") {
        ...
        if (this.formData.patientInformation.lastName === "undefined") {
          console.log("log 1");
          this.headerText =
            `Create Order: ${this.productType}™` + " - " + this.firstName;
        } else {
          console.log("log 2");
          this.headerText =
            `Create Order: ${this.productType}™` + " - " + this.firstName;
      ...
      if (fieldName === "lastName") {
        ...
        if (this.formData.patientInformation.firstName === "undefined") {
          console.log("log 3");
          this.headerText =
            `Create Order: ${this.productType}™` + " - " + this.lastName;
        } else {
          console.log("log 4");
          this.headerText =
            `Create Order: ${this.productType}™` +
            " - " +
            this.firstName +
            " " +
            this.lastName;
      ...
      if (fieldName === "insuranceType" && detail.value === "Self-Pay") {
        ...
        console.log(
          "self-pay:" +
            JSON.stringify(this.requiredFields["insurance"]["carrier"]["name"])
        );
      } else {
      ...
      if (
        ...
        (detail.value === "Workers Compensation" ||
          detail.value === "Medicaid" ||
          detail.value === "Medicare" ||
          detail.value === "Self-Pay" ||
          detail.value === "HMO (Commercial)" ||
          detail.value === "Other Government")
      ) {
      ...
      if (
        ...
        this.requiredFields["attorneyVisibleFields"][
          "nurseCaseManager"
        ] = false;
      }
  ...
  handleDiagnosisInputChange(event) {
    ...
    if (diagnosisItem.active) {
      if (!this.editedData.diagnosisMap[diagnosisType]) {
        ...
        this.editedData.diagnosisMap[diagnosisType] = {
          ...existingItem,
          ...diagnosisItem,
        };
      }
  ...
  handleMultiSelectChange(event) {
    ...
    if (event.detail.value) {
      ...
      this.editedData[event.detail.name] = this.editedData[
        event.detail.name
      ].filter((x) => x !== event.detail.option);
    }
  }
  ...
  handleRiskfactorsInputChange(event) {
    ...
    if (this.editedData.riskFactors.includes(riskfactorsKey)) {
      this.editedData.riskFactors = this.editedData.riskFactors.replace(
        riskfactorsKey,
        riskfactorsVal
      );
    } else if (
      this.editedData.riskFactors.includes(riskfactorsKeyWithoutSufix)
    ) {
      if (riskfactorsVal) {
        riskfactorsVal = riskfactorsVal.slice(0, -1);
      }
      this.editedData.riskFactors = this.editedData.riskFactors.replace(
        riskfactorsKeyWithoutSufix,
        riskfactorsVal
      );
    } else {
  ...
  validateItem(requiredNode, editDataNode, nodeName) {
    ...
    for (let key in requiredNode) {
      ...
      if (
        type == "boolean" &&
        val &&
        (!editDataNode[nodeName][key] || editDataNode[nodeName][key] == "")
      ) {
  ...
  validateInsuranceItem(insuranceName) {
    ...
    console.log("log 0.1");
    if (
      insuranceName != "primary" &&
      (!editDataNode ||
        this.isObjectEmpty(editDataNode) ||
        !editDataNode[insuranceName] ||
        this.isObjectEmpty(editDataNode[insuranceName]))
    ) {
      return missedFields;
    }
    console.log("log 0.2");
    if (
      (insuranceName == "primary" && !editDataNode) ||
      this.isObjectEmpty(editDataNode)
    ) {
    ...
    if (
      insuranceName == "primary" ||
      (typeof editDataNode[insuranceName].isOpen == "undefined" &&
        editDataNode[insuranceName].id) ||
      editDataNode[insuranceName].isOpen ||
      editDataNode[insuranceName].id
    ) {
      console.log("log 1");
      for (let key in requiredNode.carrier) {
        ...
        if (type == "boolean" && val) {
          console.log("log 1.4");
          if (
            !editDataNode[insuranceName]["carrier"] ||
            !editDataNode[insuranceName]["carrier"][key] ||
            editDataNode[insuranceName]["carrier"][key] == ""
          ) {
            console.log("log 1.5");
            missedFields.push(key);
      ...
      for (let key in requiredNode.insured) {
        ...
        if (type == "boolean" && val) {
          console.log("log 2.4");
          if (
            !editDataNode[insuranceName]["insured"] ||
            !editDataNode[insuranceName]["insured"][key] ||
            editDataNode[insuranceName]["insured"][key] == ""
          ) {
            console.log("log 2.5");
            missedFields.push(key);
  ...
  getPicklistOptions() {
    getPicklistValues()
      .then((result) => {
        ...
        this.picklistOptions.initialsurgerygraftsource =
          result.initialsurgerygraftsource;
        this.picklistOptions.revisionsurgerygraftsource =
          result.revisionsurgerygraftsource;
  ...
  validationCheck() {
    ...
    if (this.editedData.hasOwnProperty("primary")) {
      ...
      if (
        this.editedData.primary?.carrier?.hasOwnProperty("phone") &&
        carrierPhone != "" &&
        !carrierPhone.match("[0-9]{3}-[0-9]{3}-[0-9]{4}$")
      ) {
        this.dispatchEvent(
          new ShowToastEvent({
            title: "Error",
        ...
        this.editedData.primary?.insured?.hasOwnProperty(
          "socialSecurityNumber"
        ) &&
        SSN != "" &&
        !SSN.match("[0-9]{3}-[0-9]{2}-[0-9]{4}$")
      ) {
        this.dispatchEvent(
          new ShowToastEvent({
            title: "Error",
        ...
        this.editedData.primary?.insured?.hasOwnProperty("birthDate") &&
        birthDate != "" &&
        birthDate > date1
      ) {
        this.dispatchEvent(
  ...
  get productType() {
    if (
      this.editedData &&
      this.editedData.patientInformation &&
      this.editedData.patientInformation.productType
    ) {
  ...
  activateNewTab(tab) {
    ...
    let controlledAria = this.template.querySelector(
      `[aria-labelledby="${tabChild.id}"]`
    );
  ...
  deactivatePreviousTab() {
    ...
    let controlledAria = this.template.querySelector(
      `[aria-labelledby="${activeTabChild.id}"]`
    );
